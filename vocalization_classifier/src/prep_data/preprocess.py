...
def load_data(audio_path, df, sample_rate, duration, df_type):
    ...
    for _, row in progress_bar: 
        path = os.path.join(audio_path, row['class'], row['slice_file_name']) # get full file path
        try: 
...
def get_waveform_plots(audio_path, df, sample_rate, duration):
    ...
    for _, row in df.iterrows():
        ...
        if label not in class_waveforms_raw:
            file_path = os.path.join(audio_path, row['class'], row['slice_file_name'])

...
def lite_inference(val_features, interpreter, input_details, output_details):
    ...
    for i in range(len(val_features)):
        ...
        if input_dtype == np.uint8 and input_scale > 0:
            ...
            quantized_input = (float_input / input_scale + input_zero_point).astype(
                np.uint8
            )
...
def compare_models(val_features, val_labels):
    # get full model
    ...
    tflite_preds_classes = lite_inference(
        val_features, interpreter, input_details, output_details
    )  # get lite inference
    ...
    print(
        colored(
            f"Full Model Val Acc: {full_model_acc:.4f} ({round((full_model_acc*100),1)}%)\n ",
            full_acc_color,
        )
    )
    print("\n\n------------------Lite Model---------------------")
    print(
        colored(
            f"Lite Val Acc: {tflite_acc:.4f} ({round((tflite_acc*100),1)}%)\n",
            lite_acc_color,
        )
    )
